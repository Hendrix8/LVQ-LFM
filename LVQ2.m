...
s = 0.6; % window between prototypes
    ...
    for j = 1:size(ppc_list, 2)
        for k = 1:size(Ka_list, 2)
            ...
            for fold = 1: num_folds
                ...
                [W, test_acc, train_acc, val_acc] = LVQ_2(X_train, y_train, X_test, y_test, X_val, y_val, epochs, Ka, a0, s, ppc); % training using LVQ2
                ...
                test_epoch_acc = [test_epoch_acc; test_acc];
                train_epoch_acc = [train_epoch_acc; train_acc];
                val_epoch_acc = [val_epoch_acc; val_acc];

            end

            % adding the mean of the test accuracies of the 10 folds
            test_acc_graph = cat(2, test_acc_graph, mean(test_epoch_acc, 1)); 
            train_acc_graph = cat(2, train_acc_graph, mean(train_epoch_acc, 1));
            val_acc_graph = cat(2, val_acc_graph, mean(val_epoch_acc));
...
saveas(gcf, strcat('LVQ2_plot', file_name(1:4), '.pdf'), 'pdf'); 
...
annotation('textbox', [0.37 0.9 0.1 0.1], 'String', strcat('LVQ2-', file_name), 'FontSize', 10, 'FontWeight', 'bold');

saveas(gcf, strcat('LVQ2_', file_name(1:4), '.pdf'), 'pdf'); % gcf refers to the current figure
...
% LVQ2 training function
function [W, test_accs, train_accs, val_accs] = LVQ_2(X, y, X_test, y_test, X_val, y_val, epochs, Ka, a0, s, ppc )

    % initializing W
    W = init_W(X, y, ppc);

    test_accs = []; % gathering the accuracies per epoch for the graph in the end 
    train_accs = [];
    val_accs = [];
    ...
    for epoch = 1:epochs
        ...
        for i = 1:n_pat
            ...
            p_dists2 = p_dists(p_dists ~= p_dists(c1_idx)); % removing the minimum distance from the list of distances 
            
            d2 = min(p_dists2); % finding the distance of X(i) and the second closest prototype to the current X(i)
            c2_idx = find(p_dists == d2, 1); % finding the index of the second closest prototype to the currend X(i)
            
            % determining closest_1 class
            if c1_idx <= ppc
                closest1_class = [1, 0, 0];
            
            elseif c1_idx >=  ppc && c1_idx <= 2 .* ppc
                closest1_class = [0, 1, 0];
            
            else
                closest1_class = [0, 0, 1];
            end
            
            % determining closest_1 class
            if c2_idx <= ppc
                closest2_class = [1, 0, 0];
            
            elseif c2_idx >=  ppc && c2_idx <= 2 .* ppc
                closest2_class = [0, 1, 0];
            
            else
                closest2_class = [0, 0, 1];
            end
            ...
            % Checking for the three conditions 
            % CONDITION 1 : The closest prototype has to be in different class from
            % X(i)
            % CONDITION 2 : The second closest prototype has to be in the same class as
            % X(i) 
            % CONDITION 3 : X(i) must fall inside a small symmetric window around the
            % midpoint of W(i) and W(j)
            

            if ~all(Xi_class == closest1_class) & ... 
                 all(Xi_class == closest2_class) & ...
                  min([d1/d2, d2/d1] > s)

                  % UPDATE STEP
                  W(:, c1_idx) = W(:, c1_idx) - a(a0, Ka, i) .* (transpose(X(i, :)) - W(:, c1_idx) );
                  W(:, c2_idx) = W(:, c2_idx) + a(a0, Ka, i) .* (transpose(X(i, :)) - W(:, c2_idx) );
